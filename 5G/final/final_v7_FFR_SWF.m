function final_v7_FFR_SWF(~)
% show

clc
disp('----------------------------------------------------------------------------')
disp('--------------------------This is the Final Project-------------------------')
disp('-------This code is made for SWF(center)+IFR3(edge) for M-cell network------')
...
for m=1:length(snr_dB)
    for n=1:Nsim%for each sim
        ...
        p=SWF_cell(h1,sigma1(m),P1,Nc,M,U1,0,rho1);
    ...
    fprintf('For SNR = %gdB, Average network capacity with SWF(center)+IFR3(edge) is %g\n',snr_dB(m),Ave_cap_net(m));
end


switch M
    case 24
        plot(snr_dB,Ave_cap_net, 'bo-','linewidth',1.2);
        hold on
        grid on
    case 63
        plot(snr_dB,Ave_cap_net, 'bo--','linewidth',1.2);
...
legend('IFR3','IFR1+IFR3','SWF+IFR3','Location','SouthEast');
