...
for m=1:length(snr_dB)
                    ...
                    for c_=1:M
                        if rho2(c,c_)~=0 && c~=c_
                            if U1(c_)>=u && Nchannel(c_)>=u
                                I(c,u)=I(c,u)+1.2*rho2(c,c_)*P2*abs(h2(c_,c,u))^2;
                            end
                        end
                    end
    ...
    fprintf('For SNR = %gdB, Average network capacity with IFR1(center)+IFR3(edge) is %g\n',snr_dB(m),Ave_cap_net(m));
end
...
legend('IFR3','IFR1+IFR3','Location','SouthEast');
