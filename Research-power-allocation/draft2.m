...
%         average_network_cap=zeros(1,);
        ...
        for n=1:Number_realizations
            for snr_dB=1:5:100
                ...
                sigma=sqrt(P./snr);%snr=P/sigma^2; % Setting the number of active users for all the cells
                %fprintf('\n')
                %disp('--------------------------------------------------------')
                for c=1:19
            ...
            average_network_cap(snr_dB)=mean(network_cap);
            
            switch reuse_factor
                case 1
                    fprintf('average Network Capacity for a 19-cell IFR1 is %g\n',average_network_cap);
                case 3
                    fprintf('Given SNR = %d (dB), average Network Capacity for a 19-cell IFR3 is %g\n',snr_dB(m),average_network_cap(m));
            end
        end
        snr_dB=1:5:100;
        %plot the figure of IFR1 and IFR3
        switch reuse_factor
            case 1
                plot(snr_dB,average_network_cap(m), 'r^-','linewidth',1.2);
                hold on
            case 3
                plot(snr_dB,average_network_cap(m), 'cs--','linewidth',1.2);
