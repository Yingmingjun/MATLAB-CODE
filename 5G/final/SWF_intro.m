...
clc
disp('----------------------------------------------------------------------------')
disp('-----------------This code is made for SWF of M-cell network----------------')
disp('-------------------By Mingjun Ying at September 18th, 2021------------------')
disp('----------------------------------------------------------------------------')
...
%                 cap_cell(c)=sum(capacity_edge(c,1:min(U2(c),Nchannel(c))));
    ...
    fprintf('For SNR = %gdB, Average network capacity with SWF is %g\n',snr_dB(m),Ave_cap_net(m));
end
